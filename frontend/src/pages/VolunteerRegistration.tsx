...
export const VolunteerRegistration = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.name.trim() || !formData.mobile.trim()) {
      toast({
        title: t('volunteer.incompleteForm'),
        description: t('volunteer.fillAllFields'),
        variant: "destructive",
      });
      return;
    }
    ...
    if (!/^\d{10}$/.test(formData.mobile)) {
      toast({
        title: t('volunteer.invalidMobile'), 
        description: t('volunteer.validMobileNumber'),
        variant: "destructive",
      });
      return;
    }
    ...
    toast({
      title: t('volunteer.registrationSuccess'),
      description: tFormat('volunteer.thankYou', { name: formData.name }),
    });
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  id="mobile"
                  type="tel"
                  placeholder={t('volunteer.mobileHint')}
        ...
        <div className="grid md:grid-cols-2 gap-6 mt-8">
          <Card className="glass-card p-6 border-secondary/20">
            <h3 className="font-semibold text-foreground mb-3 flex items-center space-x-2">
              ...
              <span>{t('volunteer.emergencyResponse')}</span>
            </h3>
            <p className="text-sm text-muted-foreground">
              {t('volunteer.emergencyResponseDesc')}
            </p>
          </Card>
          ...
          <Card className="glass-card p-6 border-accent/20">
            <h3 className="font-semibold text-foreground mb-3 flex items-center space-x-2">
              ...
              <span>{t('volunteer.communityImpact')}</span>
            </h3>
            <p className="text-sm text-muted-foreground">
              {t('volunteer.communityImpactDesc')}
            </p>
          </Card>
        </div>
      </div>
